    ...
    for(i=0;i<N;i++)
    {
        for(j=0;j<M;j++)
        {
           // game->mapa[i][j].nivel = -1;
            if(game->mapa[i][j].pos=='.' || game->mapa[i][j].pos=='1' || game->mapa[i][j].pos=='o' || game->mapa[i][j].pos=='3' || game->mapa[i][j].pos=='2' || game->mapa[i][j].pos=='0' || game->mapa[i][j].pos=='p')
            {
                game->mapa[i][j].map=1;
            }
            ...
            else if(game->mapa[i][j].pos=='#' || game->mapa[i][j].pos=='b' || game->mapa[i][j].pos=='B')
            {
                game->mapa[i][j].map=0;
            }
            else
...
void dibujar(game_state* game)
{
    ...
    for(i=0;i<N;i++)
    {
        for(j=0;j<M;j++)
        {
            if(game->mapa[i][j].map==0 && game->mapa[i][j].pos=='#')
            {
                al_draw_filled_rectangle(j*enty,i*enty,(j+1)*enty,(i+1)*enty,al_map_rgb(0,0,255));
            }
            else if(game->mapa[i][j].map==1 && (game->mapa[i][j].pos=='.' || game->mapa[i][j].pos=='u'))
            {
            ...
            else if(game->mapa[i][j].map==1 && (game->mapa[i][j].pos=='1' || game->mapa[i][j].pos=='2' || game->mapa[i][j].pos=='3' || game->mapa[i][j].pos=='0'))
            {
                al_draw_filled_rectangle(j*enty,i*enty,(j+1)*enty,(i+1)*enty,al_map_rgb(100,0,200));
            }
            else if(game->mapa[i][j].map==1 && game->mapa[i][j].pos=='p')
            {
                al_draw_filled_rectangle(j*enty,i*enty,(j+1)*enty,(i+1)*enty,al_map_rgb(100,100,200));
            }
            else if(game->mapa[i][j].map==0 && game->mapa[i][j].pos=='B')
            {
                al_draw_filled_rectangle(j*enty,i*enty,(j+1)*enty,(i+1)*enty,al_map_rgb(255,0,0));
            }
            else if(game->mapa[i][j].map==0 && game->mapa[i][j].pos=='b')
            {
...
//=========================================================================================
    ...
    if(game->mapa[game->jug.y2][game->jug.x2].pos=='1')
    {
        game->mapa[N][M].Num_map=3;
        game->ciclo=false;
    }
    else if(game->mapa[game->jug.y2][game->jug.x2].pos=='0' && game->mapa[N][M].Num_map==4)
    {
        game->mapa[N][M].Num_map=1;
        game->ciclo=false;
    }
    else if(game->mapa[game->jug.y2][game->jug.x2].pos=='0' && game->mapa[N][M].Num_map==3)
    {
        game->mapa[N][M].Num_map=0;
        game->ciclo=false;
    }
    else if(game->mapa[game->jug.y2][game->jug.x2].pos=='0' && game->mapa[N][M].Num_map==5)
    {
        game->mapa[N][M].Num_map=2;
        game->ciclo=false;
    }
    else if(game->mapa[game->jug.y2][game->jug.x2].pos=='2')
    {
        game->mapa[N][M].Num_map=4;
        game->ciclo=false;
    }
    else if(game->mapa[game->jug.y2][game->jug.x2].pos=='3')
    {
